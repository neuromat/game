...
/*			if (eLog.realTime >= 1 && eLog.realTime < 10)
			{

				frameEEG.SetActive(true);
			...
			if (eLog.realTime >= 10 && eLog.realTime < 20)
			{
				frameEEG.SetActive(true);
...
//		Debug.Log("@ALE:++++ decisionTimeA = "+ decisionTimeA);
		...
		// @ale 190614 - Attention vs Time
/*
		if (tempoJogo >= 0 && tempoJogo < 1.5)
		{
		...
		if (tempoJogo >= 1.5 && tempoJogo >= decisionTimeA)
		{
			frameEEG.SetActive(true);
			frame1EEG.SetActive(false);
			frame2EEG.SetActive(true);
...
//				cronosOut.GetComponent<Text>().text = decisionTimeA.ToString();
				...
				timeForYellow = 0; //seta um contador para a troca da cor da cruz - 20190702
	...
	public IEnumerator WaitForNothing(float time)   //20190627 - Tempo para mudar a cor da bola de amarela para vermelha
	{
		yield return new WaitForSeconds(time);
	}	
}
